...
public class StringUtil {
    ...
    public static String arrayToString(List<String> arrs) {
        StringBuffer result = new StringBuffer("");
        if (arrs != null && arrs.size() > 0) {
            for (int i = 0; i < arrs.size(); i++) {
                ...
                if ( arrs.get(i) != null && !"".equals(arrs.get(i).trim())) {
    ...
    public static String arrayToString(List<String> arrs,String sp) {
    ...
    //把类路径转成BEAN名称
    public static String convertClassToBean( String classPath ){
        int packageDotPos = classPath.lastIndexOf(".");
        if( packageDotPos != -1 ){
            return toLowerCaseFirstOne( classPath.substring( packageDotPos+1) );
        }else{
            return toLowerCaseFirstOne(  classPath );
        }
