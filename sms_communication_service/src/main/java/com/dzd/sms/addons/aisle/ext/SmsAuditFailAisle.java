...
public class SmsAuditFailAisle  extends BaseAisle {

    private  static Logger logger = Logger.getLogger(SmsAuditFailAisle.class );

    public SmsAuditFailAisle() {
    ...
    public boolean sendSinglePackage(SmsTaskData smsTaskData, List<String> mobileList) {
        ...
        for( String p : mobileList ){
            ...
            smsSendResult.setState( Define.STATE_SENDED_AUDIT_FAIL);
            taskDataList.add(smsSendResult);

        }
        saveSendResult( taskDataList );

        logger.info("SmsAuditFailAisle sendSinglePackage state="+Define.STATE_SENDED_AUDIT_FAIL+" size="+mobileList.size());
        return false;
    }
